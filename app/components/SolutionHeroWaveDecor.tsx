...
export type SolutionHeroWaveDecorProps = {
  ...
  className?: string;
  /** When false, only soft blobs render (use with `SolutionHeroWaveBottomBand` under the hero). */
...
/** Standalone wave strip for placement directly under a hero (avoids `overflow-hidden` crop on the section). */
export function SolutionHeroWaveBottomBand({ className }: { className?: string }) {
  return (
    <div
      className={["pointer-events-none relative w-full overflow-visible bg-white border-b border-slate-200/80", className]
        .filter(Boolean)
        .join(" ")}
      aria-hidden
    >
