...
import { clmsIndustriesImagery } from "@/app/lib/clmsIndustriesImagery";
import { inopsUi } from "@/app/lib/inopsUi";

const industries = clmsIndustriesImagery.map((row) => ({ ...row }));

// ✅ Update IndustryCard to accept route and use Link
function IndustryCard({ name, imageUrl, route }: { name: string; imageUrl: string; route: string }) {
  return (
    <Link href={route} className="block"> {/* ✅ Wrap with Link */}
      <div className="industry-card group relative h-[320px] w-[320px] flex-shrink-0 overflow-hidden bg-slate-800 transition-[transform,box-shadow] duration-300 hover:-translate-y-1 hover:shadow-xl cursor-pointer">
        <Image
          src={imageUrl}
          alt={name}
          fill
...
export default function ClmsIndustriesSlider() {
  return (
    <section className="relative overflow-x-hidden py-6 lg:py-8 bg-white">
      <div className="relative mx-auto max-w-7xl px-6 lg:px-12">
        <div className="text-center">
          ...
          <h2 className={`mt-4 text-center ${inopsUi.typeSection}`}>CLMS by Industry </h2>
        ...
        <div className="flex w-max gap-0 animate-marquee-slow" style={{ willChange: "transform" }}>
          {[...industries, ...industries].map((ind, i) => (
            <IndustryCard 
              ...
              route={ind.route} // ✅ Pass the route prop
            />
          ))}
        </div>
      </div>
